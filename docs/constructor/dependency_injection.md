...
>Singleton — это же паттерн проектирования! Должно быть хорошо!"
```php
...
> Вердикт: Singleton решает проблему "единственный экземпляр", но не решает проблему управления зависимостями.

## ✅ Правильный путь: Инверсия контроля (IoC)
Ключевая идея правильного подхода — инверсия контроля.
>💡 Принцип: Вместо того чтобы Action сам создавал или искал свои зависимости, мы передаем ему эти зависимости извне.
...
> Вердикт: Отлично для небольших проектов и с малым количеством зависимостей.

## DI-контейнер
Фабрика — это хорошо, но ее можно автоматизировать. Этим занимаются DI-контейнеры.

> DI-контейнер — это "умная фабрика", которая автоматически анализирует конструкторы и методы классов, находит зависимости и создает их рекурсивно.
